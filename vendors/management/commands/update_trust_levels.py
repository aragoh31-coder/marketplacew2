...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            "--vendor-id", type=str, help="Update trust level for specific vendor ID"
        )
        parser.add_argument(
            "--stale-only",
            action="store_true",
        ...
        try:
            ...
            if dry_run:
                ...
                if trust_data:
                    ...
                    self.stdout.write(
                        self.style.ERROR(
                            f"Failed to update vendor {vendor.user.username}"
                        )
                    )
            ...
            self.stdout.write(
                self.style.ERROR(f"Error updating vendor {vendor_id}: {e}")
            )
    ...
    def update_stale_vendors(self, dry_run):
        ...
        self.stdout.write(
            f"Found {stale_vendors.count()} vendors with stale trust scores"
        )
        ...
        for vendor in stale_vendors:
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        f"Error updating vendor {vendor.user.username}: {e}"
                    )
                )
        ...
        if not dry_run:
            self.stdout.write(
                self.style.SUCCESS(
                    f"Updated {updated_count}/{stale_vendors.count()} stale vendors"
                )
            )
