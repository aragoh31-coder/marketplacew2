...
def vendor_list(request):
    vendors = Vendor.objects.filter(is_active=True, user__is_active=True).order_by(
        "-trust_level"
    )
...
def vendor_dashboard(request):
    ...
    week_revenue = vendor_orders.filter(
        status="completed", created_at__date__gte=week_ago
    ).aggregate(btc=Sum("total_btc"), xmr=Sum("total_xmr"))
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            with transaction.atomic():
                ...
                messages.success(
                    request, "Vendor application submitted! Your account is now active."
                )
                return redirect("vendors:dashboard")
    else:
...
def vendor_profile(request, vendor_id):
    ...
    total_sales = (
        Order.objects.filter(items__product__vendor=vendor, status="completed")
        .distinct()
        .count()
    )
...
def vacation_settings(request):
    ...
    if request.method == "POST":
        ...
        if action == "activate":
            ...
            if form.is_valid():
                ...
                messages.success(
                    request,
                    'Vacation mode activated. Products will show "On Vacation Listing" status.',
                )
            ...
            messages.success(
                request,
                "Vacation mode deactivated. Products are available for purchase again.",
            )
            return redirect("vendors:vacation_settings")
    ...
    return render(
        request,
        "vendors/vacation_settings.html",
        {
            "form": form,
            "vendor": vendor,
        },
...
def manage_subvendors(request):
    ...
    return render(
        request,
        "vendors/manage_subvendors.html",
        {
            "vendor": vendor,
...
def deactivate_subvendor(request, subvendor_id):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            f"Sub-vendor account {subvendor.user.username} has been deactivated.",
        )
        return redirect("vendors:manage_subvendors")

    return render(
        request,
        "vendors/deactivate_subvendor.html",
        {
            "subvendor": subvendor,
            "vendor": vendor,
        },
...
def subvendor_activity_log(request, subvendor_id):
    ...
    return render(
        request,
        "vendors/subvendor_activity_log.html",
        {
            "subvendor": subvendor,
...
def vendor_stats(request):
    ...
    vendors = (
        Vendor.objects.filter(is_approved=True, is_active=True)
        .annotate(
    ...
    total_revenue_btc = (
        vendors.aggregate(Sum("total_revenue_btc"))["total_revenue_btc__sum"] or 0
    )
    total_revenue_xmr = (
        vendors.aggregate(Sum("total_revenue_xmr"))["total_revenue_xmr__sum"] or 0
    )
