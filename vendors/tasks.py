...
def create_low_stock_notification(self, vendor_id, product_ids):
    ...
    try:
        ...
        return (
            f"Notification created for vendor {vendor_id} for {len(products)} products"
        )
...
def refresh_tor_descriptors():
    ...
    try:
        ...
        if result.returncode == 0:
            logger.info(
                "Tor service reloaded successfully - descriptors will be refreshed"
            )
            ...
            if log_check.returncode == 0 and "descriptor" in log_check.stdout.lower():
                ...
                logger.warning(
                    "Tor reloaded but no descriptor activity detected in logs"
                )
...
def recalculate_vendor_trust_levels():
    ...
    try:
        ...
        logger.info(
            f"Trust level recalculation completed: {updated_count}/{vendors.count()} vendors updated"
        )
...
def recalculate_single_vendor_trust(vendor_id):
    ...
    try:
        ...
        if trust_data:
            logger.info(
                f"Trust updated for vendor {vendor_id}: {trust_data['trust_score']}"
            )
            return {"success": True, "vendor_id": vendor_id, "trust_data": trust_data}
        else:
            return {
                "success": False,
                "vendor_id": vendor_id,
        ...
        return {"success": False, "vendor_id": vendor_id, "error": str(e)}
    ...
    try:
        ...
        for vendor in stale_vendors:
            try:
                ...
                logger.error(
                    f"Failed to update stale trust for vendor {vendor.id}: {e}"
                )

        logger.info(
            f"Stale trust update completed: {updated_count}/{stale_vendors.count()} vendors updated"
        )
