...
class VendorBond(models.Model):
    ...
    vendor = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="vendor_bond"
    )
    bond_amount = models.DecimalField(
        max_digits=16, decimal_places=8, validators=[MinValueValidator(Decimal("0.01"))]
    )
    currency = models.CharField(
        max_length=3, choices=[("btc", "Bitcoin"), ("xmr", "Monero")]
    )
    status = models.CharField(
        max_length=20, choices=BOND_STATUS_CHOICES, default="pending"
    )
...
class BroadcastMessage(models.Model):
    ...
    message_type = models.CharField(
        max_length=20, choices=MESSAGE_TYPE_CHOICES, default="info"
    )
    ...
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="created_broadcasts"
    )
    created_at = models.DateTimeField(auto_now_add=True)
...
class MessageAlert(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="message_alerts"
    )
...
class AdminSecurityLog(models.Model):
    ...
    admin_user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="admin_security_logs"
    )
