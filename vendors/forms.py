...
class VendorApplicationForm(forms.ModelForm):
    terms_accepted = forms.BooleanField(
        required=True, label="I accept the vendor terms and conditions"
    )
    ...
    def save(self, commit=True, user=None):
        ...
        if user:
            ...
            try:
                ...
                raise forms.ValidationError(
                    "User must be an approved vendor to create products"
                )
        ...
        if image and user:
            ...
            if success:
                if instance.pk and (
                    instance.image_filename or instance.thumbnail_filename
                ):
                    processor.delete_images(
                        instance.image_filename, instance.thumbnail_filename
                    )
...
class SubVendorForm(forms.ModelForm):
    username = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-input", "placeholder": "support1"}
        ),
