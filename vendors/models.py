...
class Vendor(PrivacyModel):
    ...
    bond_currency = models.CharField(
        max_length=3, choices=[("BTC", "Bitcoin"), ("XMR", "Monero")], default="BTC"
    )
    ...
    def update_trust_level_async(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to trigger async trust update for vendor {self.id}: {e}"
            )
...
class Promotion(PrivacyModel):
    ...
    vendor = models.ForeignKey(
        Vendor, on_delete=models.CASCADE, related_name="promotions"
    )
...
class Feedback(PrivacyModel):
    ...
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    vendor = models.ForeignKey(
        Vendor, on_delete=models.CASCADE, related_name="feedback"
    )
...
class SubVendor(PrivacyModel):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    main_vendor = models.ForeignKey(
        Vendor, on_delete=models.CASCADE, related_name="sub_vendors"
    )
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, related_name="sub_vendor"
    )
    created_by = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="created_subvendors"
    )
...
class SubVendorActivityLog(PrivacyModel):
    ...
    sub_vendor = models.ForeignKey(
        SubVendor, on_delete=models.CASCADE, related_name="activity_logs"
    )
