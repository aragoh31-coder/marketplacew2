...
class AntiDDoSStressTester:
    ...
    def test_rapid_fire_requests(self, num_requests=1000, concurrent=50):
        ...
        logger.info(f"Starting rapid fire test: {num_requests} requests, {concurrent} concurrent")
        ...
        def make_request():
            try:
                ...
                response = requests.get(
                    f"{self.base_url}/",
                    headers={'User-Agent': self.generate_bot_user_agent()},
                    timeout=10,
                ...
                if response.status_code == 302 and 'anti_ddos' in response.headers.get('Location', ''):
                    results['blocked'] += 1
    ...
    def test_low_entropy_solutions(self, num_attempts=100):
        ...
        def attempt_low_entropy():
            try:
                ...
                response = session.get(f"{self.base_url}/anti_ddos/challenge/")
                if response.status_code != 200:
                    results['error'] += 1
                    return
                
                csrf_token = self.extract_csrf_token(response.text)
                challenge_data = self.extract_challenge_data(response.text)
                start_ts = int(time.time())
                
                time.sleep(2.1)  # Wait minimum solve time
                
                verify_response = session.post(f"{self.base_url}/anti_ddos/verify/", data={
                    'csrfmiddlewaretoken': csrf_token,
                    'challenge_data': challenge_data,
                    'start_time': str(start_ts),
                    'answer': '1',  # Low entropy answer
                    **{f'hp{i}': '' for i in range(5)}
                })
                ...
                if verify_response.status_code == 302:
                    if 'anti_ddos' in verify_response.headers.get('Location', ''):
                        results['blocked'] += 1
                    else:
                        results['success'] += 1
                else:
        ...
        with ThreadPoolExecutor(max_workers=10) as executor:
            futures = [executor.submit(attempt_low_entropy) for _ in range(num_attempts)]
            for future in futures:
                future.result()
    ...
    def test_legitimate_users(self, num_users=50):
        ...
        def simulate_legitimate_user():
            try:
                ...
                response = session.get(
                    f"{self.base_url}/anti_ddos/challenge/",
                    headers={'User-Agent': self.generate_legitimate_user_agent()}
                )
                ...
                csrf_token = self.extract_csrf_token(response.text)
                challenge_data = self.extract_challenge_data(response.text)
                correct_answer = self.solve_challenge(response.text)
                start_ts = int(time.time())
        ...
        with ThreadPoolExecutor(max_workers=5) as executor:
            futures = [executor.submit(simulate_legitimate_user) for _ in range(num_users)]
            for future in futures:
                future.result()
        ...
        self.results.append(result)
        logger.info(f"Legitimate user test complete: {false_positive_rate:.1f}% false positives")
        return result
    ...
    def test_honeypot_triggering(self, num_attempts=100):
        ...
        def trigger_honeypot():
            try:
                ...
                honeypot_data = {f'hp{i}': f'bot_value_{i}' for i in range(5)}
    ...
    def generate_report(self):
        ...
        legitimate_test = next((r for r in self.results if r.test_type == "Legitimate Users"), None)
        ...
        if attack_tests:
            ...
            overall_blocking = (total_blocked / total_attack_requests) * 100 if total_attack_requests > 0 else 0
        ...
        blocking_target_met = report['summary']['overall_blocking_effectiveness'] >= 95.0
        false_positive_target_met = report['summary']['false_positive_rate'] <= 5.0
        report['summary']['target_met'] = blocking_target_met and false_positive_target_met
        ...
        with open('/home/ubuntu/marketplace/STRESS_TEST_REPORT.json', 'w') as f:
...
"""
        ...
        with open('/home/ubuntu/marketplace/STRESS_TEST_REPORT.txt', 'w') as f:
            f.write(report_text)
