...
class BotDetectionMiddleware:
    ...
    def get_client_identifier(self, request):
        """Get session-based client identifier for Tor compatibility"""
        session_hash = get_session_hash(request)
        user_agent = request.META.get("HTTP_USER_AGENT", "")

        identifier = hashlib.sha256(f"{session_hash}:{user_agent}".encode()).hexdigest()[:16]
        return identifier
    ...
    def check_rate_limits(self, request, client_id):
        ...
        minute_key = (
            f"requests_minute:{client_id}:{current_time.strftime('%Y%m%d%H%M')}"
        )
        ...
        if "/accounts/login/" in request.path:
            login_key = (
                f"login_attempts:{client_id}:{current_time.strftime('%Y%m%d%H')}"
            )
        ...
        if "/accounts/register/" in request.path:
            reg_key = (
                f"registration_attempts:{client_id}:{current_time.strftime('%Y%m%d%H')}"
            )
    ...
    def flag_suspicious_activity(self, client_id, reason):
        ...
        flags = [
            f for f in flags if timezone.datetime.fromisoformat(f["timestamp"]) > cutoff
        ]
        ...
        if len(flags) >= 5:
            ...
            logger.error(
                f"Blocking client {client_id} due to suspicious activity: {flags}"
            )
