...
class OptimizedSecurityMiddleware:
    ...
    def __call__(self, request):
        ...
        if self.should_block_request(request, session_hash):
    ...
    def get_request_signature(self, request):
        ...
        signature_data = (
            f"{request.path}|{request.method}|{get_session_hash(request)}"
        )
        secret = getattr(settings, 'REQUEST_SIG_SECRET', settings.SECRET_KEY).encode()
        return hmac.new(secret, signature_data.encode(), hashlib.sha256).hexdigest()[:12]

    def should_block_request(self, request, session_hash):
        ...
        if self.exceeds_rate_limits(session_hash):
            return True
    ...
    def exceeds_rate_limits(self, client_ip):
        ...
        for window_name, duration, limit in windows:
            if self.check_rate_window(
                client_ip, window_name, duration, limit, current_time
            ):
                return True
