...
class NoJSCaptchaMixin:
    ...
    def get_session_hash(self, request):
    ...
    def _check_rate_limit(self):
        ...
        try:
            current_time = time.time()
            session_id = self.get_session_hash(self.request)
    ...
    def clean(self):
        ...
        if self.request:
            ...
            if not captcha_validated or (time.time() - captcha_time) > 300:
                raise ValidationError(
                    "Please complete the visual CAPTCHA verification."
                )
...
class BotChallengeForm(forms.Form):
    ...
    def clean(self):
        ...
        if (
            ...
            and cleaned_data.get("challenge_answer") != self.expected_answer
        ):
            raise ValidationError("Incorrect answer")
...
class TripleAuthForm(forms.Form):
    ...
    password = forms.CharField(
        widget=forms.PasswordInput(
            attrs={"class": "form-control", "placeholder": "Admin Password"}
        ),
    ...
    secondary_password = forms.CharField(
        widget=forms.PasswordInput(
            attrs={"class": "form-control", "placeholder": "Secondary Password"}
        ),
    ...
    def clean_secondary_password(self):
        ...
        if secondary == main_password:
            raise forms.ValidationError(
                "Secondary password must be different from main password."
            )
