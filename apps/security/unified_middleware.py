...
class UnifiedSecurityMiddleware:
    ...
    def __call__(self, request):
        ...
        if not rate_limit_result["allowed"]:
            return HttpResponse(
                f"Rate limit exceeded: {rate_limit_result['reason']}", status=429
            )
    ...
    def check_rate_limits(self, client_id, request):
        ...
        rate_limits = security_config.get(
            "RATE_LIMITS",
            {"requests_per_minute": 50, "requests_per_hour": 500, "burst_limit": 20},
        )
    ...
    def is_bot_request(self, request):
        ...
        if any(
            sus_path in path
            for sus_path in ["/wp-admin/", "/phpmyadmin/", ".php", ".asp"]
        ):
            return True
        ...
        missing_headers = sum(
            1 for header in browser_headers if not request.META.get(header)
        )
    ...
    def analyze_threat_landscape(self, request, circuit_id):
        ...
        current_time = time.time()
        analysis = {"score": 0, "factors": [], "action": "allow", "reason": "clean"}
    ...
    def analyze_circuit_behavior(self, circuit_id, current_time):
        ...
        behavior = cache.get(
            cache_key,
            {"request_times": [], "first_seen": current_time, "request_count": 0},
        )
        ...
        behavior["request_times"] = [
            t for t in behavior["request_times"] if current_time - t < 600
        ]
        ...
        recent_burst = sum(
            1 for t in behavior["request_times"] if current_time - t < 60
        )
        if recent_burst > 10:
            score += 20
    ...
    def classify_attack_vector(self, request):
        ...
        if any(
            ...
            for pattern in ["union", "select", "drop"]
        ):
    ...
    def record_block(self, request, circuit_id, reason):
        ...
        logger.warning(
            f"Unified security block: {reason} | Circuit: {circuit_id[:8]} | Path: {request.path}"
        )
    ...
    def _update_load_metrics(self):
        ...
        try:
            ...
            load_level = self._calculate_load_level(
                cpu_percent, memory.percent, connections
            )
