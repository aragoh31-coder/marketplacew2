...
class SecurityPerformanceMonitor:
    ...
    def record_request(self, request, blocked=False, response_time=None):
        ...
        if blocked:
            ...
            self.metrics['blocked_ips'].add(client_ip)
        else:
            self.metrics['requests_allowed'] += 1
            self.metrics['active_ips'].add(client_ip)
    ...
    def _monitor_loop(self):
        ...
        while self.monitoring:
            try:
                ...
                time.sleep(getattr(settings, 'SECURITY_MONITORING', {}).get('METRICS_INTERVAL', 30))
    ...
    def _check_alert_thresholds(self):
        ...
        thresholds = getattr(settings, 'SECURITY_MONITORING', {}).get('ALERT_THRESHOLDS', {})
        ...
        if total_requests > 0:
            ...
            if self.metrics['avg_response_time'] > thresholds.get('response_time_ms', 500):
                logger.warning(f"High response time: {self.metrics['avg_response_time']:.1f}ms")
    ...
    def _save_metrics_snapshot(self):
        ...
        snapshot = {
            ...
            'block_rate_percent': (self.metrics['requests_blocked'] / max(1, self.metrics['requests_total'])) * 100,
    ...
    def _detect_attack_vector(self, request, blocked):
        ...
        self.metrics['attack_vectors'][vector] = self.metrics['attack_vectors'].get(vector, 0) + 1
