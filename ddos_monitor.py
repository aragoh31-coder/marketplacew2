    ...
    def check_openresty_logs(self):
        try:
            result = subprocess.run(
                ["docker", "compose", "logs", "--tail=20", "openresty"],
                capture_output=True,
                text=True,
            ...
            self.attack_patterns["rate_limit_exceeded"] += len(
                re.findall(r"429|Rate limit exceeded", logs)
            )
            self.attack_patterns["bot_detected"] += len(
                re.findall(r"403.*Bot detected", logs)
            )
            self.attack_patterns["suspicious_paths"] += len(
                re.findall(r"Suspicious request blocked", logs)
            )
            self.attack_patterns["ip_blocks"] += len(re.findall(r"Blocked IP:", logs))
            self.attack_patterns["pow_challenges"] += len(
                re.findall(r"PoW required", logs)
            )
