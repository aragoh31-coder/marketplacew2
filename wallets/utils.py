...
def check_rate_limit(request, action, max_attempts=5, window=3600):
    ...
    if attempts >= max_attempts:
        logger.warning(
            f"Rate limit exceeded for user {request.user.username} on action {action}"
        )
        return False
...
def send_withdrawal_notification(withdrawal_request):
    ...
    try:
        ...
        message = render_to_string(
            "wallets/emails/withdrawal_submitted.txt",
            {"withdrawal": withdrawal_request, "user": withdrawal_request.user},
        )
        ...
        admin_message = render_to_string(
            "wallets/emails/withdrawal_admin.txt", {"withdrawal": withdrawal_request}
        )
...
def get_session_hash(request):
    """Get session-based identifier for privacy-focused tracking"""
    from core.utils.security import get_session_hash as core_get_session_hash
    return core_get_session_hash(request)
...
def validate_withdrawal_security(user, amount, currency, address):
    ...
    large_amount_thresholds = {"btc": Decimal("0.1"), "xmr": Decimal("10.0")}
    ...
    previous_use = WithdrawalRequest.objects.filter(
        user=user, address=address, status="completed"
    ).exists()
