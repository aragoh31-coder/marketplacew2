...
def check_suspicious_activity():
    ...
    try:
        ...
        suspicious_ips = WalletAuditLog.objects.filter(
        ...
        if suspicious_ips.exists():
            logger.warning(f"Suspicious activity detected: {suspicious_ips.count()} IPs with rapid withdrawals")
            
            for user_data in suspicious_ips:
                logger.warning(f"User {user_data['user']} made {user_data['count']} withdrawal requests in 1 hour")
...
def monitor_wallet_security():
    ...
    try:
        from .models import WithdrawalRequest, WalletAuditLog
        ...
        audit_stats = WalletAuditLog.objects.filter(
            timestamp__gte=cutoff_time
...
def reconcile_wallet_balances():
    ...
    try:
        ...
        for wallet in wallets:
            try:
                logger.info(f"Reconciling wallet for user {wallet.user.username}: BTC={wallet.btc_balance}, XMR={wallet.xmr_balance}")
                reconciled_count += 1
                
            except Exception as e:
                logger.error(f"Failed to reconcile wallet for user {wallet.user.username}: {str(e)}")
                discrepancies.append(wallet.user.id)
        
        logger.info(f"Wallet reconciliation completed: {reconciled_count} wallets checked")
        if discrepancies:
