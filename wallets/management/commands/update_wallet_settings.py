...
'''
        ...
        try:
            ...
            if 'WALLET_SECURITY' not in content:
                ...
                self.stdout.write(
                    self.style.SUCCESS('Successfully added wallet security configuration to settings.py')
                )
            else:
                self.stdout.write(
                    self.style.WARNING('Wallet security configuration already exists in settings.py')
                )
        except Exception as e:
            self.stdout.write(
                self.style.ERROR(f'Error updating settings.py: {str(e)}')
            )
