...
from wallets.models import Transaction, Wallet, WalletBalanceCheck
    ...
    def handle(self, *args, **options):
        ...
        for wallet in wallets:
            try:
                wallets_checked += 1
                self.stdout.write(
                    f"Checking wallet {wallet.id} for user {wallet.user.username}"
                )

                btc_transactions = Transaction.objects.filter(
                    user=wallet.user, currency="btc"
                ).aggregate(
                    deposits=Sum(
                        "amount", filter=Q(type__in=["deposit", "conversion"])
                    ),
                ...
                expected_btc = (btc_transactions["deposits"] or Decimal("0")) - (
                    btc_transactions["withdrawals"] or Decimal("0")
                )
                expected_xmr = (xmr_transactions["deposits"] or Decimal("0")) - (
                ...
                discrepancy_found = (
                    btc_diff > Decimal("0.00000001")
                    or xmr_diff > Decimal("0.000000000001")
                    or escrow_btc_diff > Decimal("0.00000001")
                    or escrow_xmr_diff > Decimal("0.000000000001")
                )

                if discrepancy_found:
                    discrepancies_found += 1
                    self.stdout.write(
                        self.style.WARNING(f"Discrepancy found in wallet {wallet.id}:")
                    )
                    self.stdout.write(
                        f"  BTC: Expected {expected_btc}, Actual {wallet.balance_btc}, Diff {btc_diff}"
                    )
                    self.stdout.write(
                        f"  XMR: Expected {expected_xmr}, Actual {wallet.balance_xmr}, Diff {xmr_diff}"
                    )
                    self.stdout.write(
                        f"  Escrow BTC: Expected {expected_escrow_btc}, Actual {wallet.escrow_btc}, Diff {escrow_btc_diff}"
                    )
                    self.stdout.write(
                        f"  Escrow XMR: Expected {expected_escrow_xmr}, Actual {wallet.escrow_xmr}, Diff {escrow_xmr_diff}"
                    )
                    ...
                    if options["fix_discrepancies"]:
                        if btc_diff <= Decimal("0.00001") and xmr_diff <= Decimal(
                            "0.001"
                        ):
                ...
                self.stdout.write(
                    self.style.ERROR(f"Error checking wallet {wallet.id}: {str(e)}")
                )
