...
class WithdrawalForm(forms.Form):
    ...
    currency = forms.ChoiceField(
        choices=CURRENCY_CHOICES, widget=forms.Select(attrs={"class": "form-control"})
    )
    ...
    amount = forms.DecimalField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control", "step": "0.000000000001"}
        ),
    )
    ...
    address = forms.CharField(
        ...
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Cryptocurrency address"}
        ),
    )
    ...
    def clean_address(self):
        ...
        if currency == "btc":
            if not re.match(
                r"^[13][a-km-zA-HJ-NP-Z1-9]{25,34}$|^bc1[a-z0-9]{39,59}$", address
            ):
