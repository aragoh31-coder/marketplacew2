...
class MarketplaceFeeCalculator:
    ...
    def get_fee_rate(cls, fee_type):
        ...
        try:
            return getattr(
                settings,
                f"MARKETPLACE_{fee_type.upper()}",
                cls.DEFAULT_FEES.get(fee_type, Decimal("0")),
            )
    ...
    def calculate_marketplace_fee(cls, order_amount, vendor_tier="standard"):
        ...
        try:
            ...
            fee_amount = (order_amount * fee_rate).quantize(
    ...
    def calculate_withdrawal_fee(cls, amount, currency="BTC"):
        ...
        try:
            ...
            return {"fee_amount": Decimal("0.001"), "error": str(e)}
    ...
    def calculate_listing_fee(cls, product_data, is_premium=False):
        ...
        try:
            ...
            final_fee = (fee * category_multiplier).quantize(
                Decimal("0.00000001"), rounding=ROUND_HALF_UP
            )
    ...
    def calculate_dispute_fee(cls, order_amount):
        ...
        try:
            ...
            return {
                "fee_amount": dispute_fee.quantize(
                    Decimal("0.00000001"), rounding=ROUND_HALF_UP
                ),
                "base_fee": base_fee,
                "percentage_fee": percentage_fee,
            ...
            return {"fee_amount": cls.DEFAULT_FEES["dispute_fee"], "error": str(e)}
    ...
    def calculate_total_fees(
        ...
        try:
            ...
            payment_fee = order_amount * (
                cls.get_fee_rate("payment_processing_fee") / 100
            )
            ...
            return {
                ...
                "payment_fee": payment_fee.quantize(
                    Decimal("0.00000001"), rounding=ROUND_HALF_UP
                ),
                "withdrawal_fee": (
                    withdrawal_fee["fee_amount"] if include_withdrawal else Decimal("0")
                ),
                "fee_breakdown": {
                    "marketplace": marketplace_fee,
                    "payment": {
                        "fee_amount": payment_fee,
                        "rate": cls.get_fee_rate("payment_processing_fee"),
                    },
            ...
            return {"total_fees": Decimal("0"), "error": str(e)}
