...
class EnhancedErrorHandlingMiddleware:
    ...
    def _generate_error_id(self, request, exception):
        ...
        error_data = (
            f"{request.path}:{type(exception).__name__}:{timezone.now().isoformat()}"
        )
    ...
    def _log_error(self, request, exception, error_id):
        ...
        try:
            ...
            logger.error(
                f"Unhandled exception [{error_id}]: {exception}", extra=error_context
            )
    ...
    def _handle_security_error(self, request, exception, error_id):
        ...
        try:
            ...
            if error_count >= 5:
                logger.warning(
                    f"Multiple security errors from {client_ip}: {error_count}"
                )
    ...
    def _get_client_identifier(self, request):
        ...
        if hasattr(request, 'session') and request.session.session_key:
            return f"session_{request.session.session_key[:16]}"
        ...
        try:
            context = {
                ...
                "support_contact": getattr(
                    settings, "SUPPORT_EMAIL", "support@marketplace.onion"
                ),
            }
