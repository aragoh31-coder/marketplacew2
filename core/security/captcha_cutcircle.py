...
def generate_cut_circle():
    ...
    for i in range(SEGMENTS):
        ...
        draw.pieslice(
            ...
            start_angle, end_angle, fill=fill_color, outline=outline_color
        )
...
class OneClickCaptcha:
    ...
    def generate(self):
        ...
        for i in range(self.num_circles):
            attempts = 0
            while attempts < 50:
                x = random.randint(self.circle_radius + 10, self.width - self.circle_radius - 10)
                y = random.randint(self.circle_radius + 10, self.height - self.circle_radius - 10)
...
def validate_captcha(request, click_x, click_y):
    ...
    try:
        ...
        print(f"DEBUG: Stored circle at ({circle_x}, {circle_y}) with radius {radius}", flush=True)
        ...
        for scale in scaling_factors:
            ...
            if 0 <= image_x <= actual_width and 0 <= image_y <= actual_height:
                ...
                print(f"DEBUG: Scale {scale}: transformed ({image_x:.1f}, {image_y:.1f}), distance {distance:.1f}", flush=True)
                
                if distance <= radius * 3.0:
                    print(f"DEBUG: MATCH! Scale {scale} with distance {distance:.1f} <= {radius * 3.0}", flush=True)
                    request.session.pop('captcha', None)
                    return True
        
        distance = math.sqrt((browser_x - circle_x)**2 + (browser_y - circle_y)**2)
        print(f"DEBUG: Direct distance check: {distance:.1f} <= {radius * 3.0}", flush=True)
        if distance <= radius * 3.0:
