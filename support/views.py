...
def support_home(request):
    ...
    if request.user.is_authenticated:
        recent_tickets = SupportTicket.objects.filter(user=request.user).order_by(
            "-created_at"
        )[:5]
...
def ticket_list(request):
    tickets = SupportTicket.objects.filter(user=request.user).select_related("user").order_by("-created_at")
