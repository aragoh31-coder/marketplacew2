...
class TorPowLauncher:
    def __init__(self):
        ...
        self.session.request = lambda *args, **kwargs: original_request(
            *args, timeout=SOLVE_TIMEOUT, **kwargs
        )
        ...
        try:
            if use_clearnet:
                response = self.session.post(
                    url, data=data, allow_redirects=False, proxies={}
                )
            else:
                proxies = {
                    "http": f"socks5h://localhost:{TOR_SOCKS_PORT}",
            ...
            if response.status_code == 302:
                ...
                if location == "/" or location.endswith("/"):
                    ...
                    if hmac_token:
                        logger.info(
                            f"HMAC token stored in session: {len(hmac_token)} chars"
                        )
                    else:
                    ...
                    logger.warning(
                        f"Challenge solution rejected - redirected to: {location}"
                    )
                    return False
            else:
    ...
    def run_challenge_cycle(self, use_clearnet=False):
        ...
        try:
            ...
            if current_time < mint_time:
                ...
                logger.info(
                    f"Waiting {wait_time:.1f}s for minimum solve time (mint={mint_time})"
                )
        ...
        success = self.submit_solution(
            challenge_data, start_time, solution, use_clearnet
        )
        if not success:
            return False
    ...
    for mode_name, use_clearnet in modes:
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            try:
                ...
                if success:
                    logger.info(
                        f"✅ PoW challenge completed successfully via {mode_name}!"
                    )
                ...
                logger.error(
                    f"Attempt {attempt} failed with exception ({mode_name}): {e}"
                )
