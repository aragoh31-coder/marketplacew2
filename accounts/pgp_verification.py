...
def pgp_settings(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            request.session['pgp_verification_code_hash'] = sha256(verification_code.encode()).hexdigest()
...
def pgp_verify_key(request):
    ...
    if request.method == 'POST':
        ...
        if not all([stored_code_hash, expires, temp_key]):
        ...
        if not expires_at or timezone.now() > expires_at:
            ...
            for key in ['pgp_verification_code_hash', 'pgp_verification_expires', 
                       'temp_pgp_key', 'temp_pgp_fingerprint', 'temp_pgp_login_enabled']:
                request.session.pop(key, None)
            return redirect('accounts:pgp_settings')
        
        user_input_hash = sha256(submitted_code.encode()).hexdigest()
