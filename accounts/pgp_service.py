...
"""
    ...
    def validate_key_format(self, key_data):
        ...
        has_begin = any(
            re.search(pattern, key_data, re.IGNORECASE) for pattern in begin_patterns
        )
        has_end = any(
            re.search(pattern, key_data, re.IGNORECASE) for pattern in end_patterns
        )
        ...
        for line in lines:
            line = line.strip()
            if any(
                re.search(pattern, line, re.IGNORECASE) for pattern in begin_patterns
            ):
                in_key_block = True
                normalized_lines.append("-----BEGIN PGP PUBLIC KEY BLOCK-----")
            elif any(
                re.search(pattern, line, re.IGNORECASE) for pattern in end_patterns
            ):
                in_key_block = False
        ...
        for subkey in subkeys:
            if isinstance(subkey, dict):
                formatted_subkey = {
                    ...
                    "created": (
                        datetime.fromtimestamp(int(subkey.get("date", 0))).strftime(
                            "%Y-%m-%d"
                        )
                        if subkey.get("date")
                        else "Unknown"
                ...
                formatted_subkey = {
                    ...
                    "algorithm": (
                        self._get_algorithm_name(subkey[3])
                        if len(subkey) > 3
                        else "Unknown"
                    ),
                    ...
                    "created": (
                        datetime.fromtimestamp(int(subkey[4])).strftime("%Y-%m-%d")
                        if len(subkey) > 4 and subkey[4]
                        else "Unknown"
                    ),
                }
            else:
    ...
    def import_public_key(self, public_key_data):
        ...
        try:
            ...
            if import_result.count > 0:
                ...
                logger.info(
                    f"Successfully imported key with fingerprint: {fingerprint}"
                )
                ...
                if key_info and key_info.get("success"):
                    ...
                    if not caps.get("can_encrypt") and not caps.get(
                ...
                error_msg = (
                    str(import_result.stderr)
                    if hasattr(import_result, "stderr")
                    else "Unknown error"
                )
                ...
                if "invalid" in error_msg.lower():
                    ...
                    return {"success": False, "error": "This key is already imported"}
                else:
                    return {
                        "success": False,
    ...
    def encrypt_message(self, message, recipient_fingerprint):
        ...
        try:
            ...
            if encrypted_data.ok:
                ...
                logger.debug(
                    f"Encrypted message length: {len(encrypted_message)} bytes"
                )
    ...
    def verify_signature(self, signed_message):
        ...
        try:
            ...
            if verified.valid:
                logger.info(
                    f"Signature verified successfully for key: {verified.key_id}"
                )
                return {
                    "success": True,
                    "valid": True,
        ...
        try:
            ...
            try:
                ...
                if verification_result.get("valid"):
                    ...
                    logger.warning(
                        "Signature verification failed, but message extracted"
                    )
            except Exception as verify_error:
                logger.warning(
                    f"Signature verification error (message still extracted): {verify_error}"
                )
    ...
    def get_key_info(self, fingerprint):
        ...
        try:
            ...
            for key in keys:
                if key["fingerprint"] == fingerprint:
                    ...
                    return {
                        ...
                        "created": (
                            datetime.fromtimestamp(int(key.get("date", 0))).strftime(
                                "%Y-%m-%d"
                            )
                            if key.get("date")
                            else "Unknown"
                        ...
                        "expires": (
                            datetime.fromtimestamp(int(key.get("expires", 0))).strftime(
                                "%Y-%m-%d"
                            )
                            if key.get("expires")
                            else "Never"
            ...
            return {"success": False, "error": f"Key info error: {str(e)}"}
    ...
    def create_signed_message(self, message, keyid=None, passphrase=None):
        ...
        try:
            signed_data = self.gpg.sign(
                message, keyid=keyid, passphrase=passphrase, clearsign=True
            )
            ...
            return {"success": False, "error": f"Signing error: {str(e)}"}
