...
def login_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if user:
                need_totp = bool(getattr(user, "totp_enabled", False))
                need_pgp = bool(getattr(user, "pgp_login_enabled", False) and getattr(user, "pgp_public_key", None))
                request.session["pre_auth_user"] = user.id
                request.session["need_totp"] = need_totp
                request.session["need_pgp"] = need_pgp
...
def pgp_challenge_view(request):
    ...
    if request.method == "POST":
        ...
        if challenge_code and user.verify_pgp_challenge(challenge_code):
            pre_id = request.session.get("pre_auth_user")
            if pre_id and int(pre_id) == int(user.id):
                request.session["need_pgp"] = False
                if request.session.get("need_totp"):
                    messages.info(request, "PGP verified. Complete TOTP to finish login.")
                    return redirect("accounts:verify_totp")

            if not request.session.get("need_totp") and not request.session.get("need_pgp"):
                login(request, user)
                for k in ("pre_auth_user", "need_totp", "need_pgp"):
                ...
                messages.success(request, "2FA complete. Welcome back!")
                return redirect("/")
        else:
...
def verify_totp(request):
    ...
    if request.method == "POST":
        ...
        if verified:
            ...
            request.session["need_totp"] = False
            request.session["pre_auth_user"] = user.id
            if request.session.get("need_pgp"):
                messages.info(request, "Authenticator verified. Complete PGP challenge to finish login.")
                return redirect("accounts:pgp_challenge")

            login(request, user)
            for k in ("pre_auth_user", "need_totp", "need_pgp", "totp_verified"):
                request.session.pop(k, None)
            return redirect("/")
        else:
            with transaction.atomic():
