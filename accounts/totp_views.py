...
def totp_verify(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            pre_auth_user = request.session.get("pre_auth_user")
            need_pgp = request.session.get("need_pgp")
            if pre_auth_user and int(pre_auth_user) == int(user.id):
                ...
                from django.contrib.auth import login
                login(request, user)
                for k in ("pre_auth_user", "need_totp", "need_pgp", "totp_user_id", "totp_required"):
