...
class OrderIDGenerator:
    ...
    def generate_tracking_reference(order_reference, vendor_id=None):
        ...
        if cache.get(cache_key):
            return OrderIDGenerator.generate_tracking_reference(order_reference, vendor_id)
    ...
    def generate_dispute_reference(order_reference):
        ...
        timestamp = str(int(timezone.now().timestamp()))
        hash_part = hashlib.sha256(f"{order_reference}:{timestamp}".encode()).hexdigest()[:6].upper()
