...
def admin_login(request):
    ...
    if request.method == 'POST':
        ...
        print(f"DEBUG: About to validate admin captcha with coordinates ({click_x}, {click_y})", flush=True)
...
def withdrawal_approve(request, withdrawal_id):
    ...
    if request.method == 'POST':
        ...
        expected_challenge = request.session.get(f'withdrawal_challenge_{withdrawal_id}')
            ...
            if expected_challenge in pgp_response:
                with transaction.atomic():
                    ...
                    if withdrawal.status in ['pending', 'reviewing']:
                        ...
                        audit_hmac = hashlib.sha256(f"AUDIT_KEY:{audit_data}".encode()).hexdigest()
                        withdrawal.admin_notes = f"PGP verified. HMAC: {audit_hmac[:16]}"
                        
                        withdrawal.save()
                        
                        request.session.pop(f'withdrawal_challenge_{withdrawal_id}', None)
                        ...
                        messages.success(request, f'Withdrawal #{withdrawal_id} approved successfully with PGP verification.')
                ...
                messages.error(request, 'PGP challenge verification failed - invalid response.')
        else:
            messages.error(request, 'PGP challenge verification failed - missing or invalid data.')
