...
def admin_login(request):
    ...
    if request.method == "GET":
        ...
        form = AdminLoginForm()
        return render(
            request, "adminpanel/login.html", {"form": form, "captcha_img": img_b64}
        )
    ...
    if request.method == "POST":
        ...
        print(
            f"DEBUG: About to validate admin captcha with coordinates ({click_x}, {click_y})",
            flush=True,
        )
        captcha_result = validate_captcha(request, click_x, click_y)
        print(f"DEBUG: Admin captcha validation result: {captcha_result}", flush=True)

        if not captcha_result:
            print(f"DEBUG: Admin CAPTCHA validation failed, regenerating", flush=True)
            img_b64 = init_captcha_session(request)
        ...
        if form.is_valid():
            ...
            if user and user.is_superuser:
                ...
                failed_attempts = (
                    request.session.get(f"admin_failed_attempts_{username}", 0) + 1
                )
...
def initiate_triple_auth(request, user):
    ...
    if not challenge_data["success"]:
        messages.error(
            request, "PGP challenge generation failed. Please contact support."
        )
...
def handle_triple_auth(request):
    ...
    if form.is_valid():
        ...
        messages.success(
            request, "Successfully authenticated with triple authentication."
        )
        ...
        if form.is_valid():
            ...
            if provided_hash == expected_secondary:
                ...
                if failed_attempts >= 3:
                    request.session.flush()
                    messages.error(
                        request, "Too many failed secondary authentication attempts."
                    )
                    return redirect("adminpanel:admin_login")
        else:
...
def pgp_verify(request):
    ...
    if request.method == "POST":
            ...
            if challenge:
                ...
                if verify_result["success"]:
                    ...
                    request.session.set_expiry(
                        settings.ADMIN_PANEL_CONFIG["SESSION_TIMEOUT"]
                    )
        ...
        challenge = (
            f"Admin login challenge for {user.username} at {timezone.now().isoformat()}"
        )
    ...
    return render(
        ...
        "adminpanel/pgp_verify.html",
        {
            ...
            "user": user,
            "encrypted_challenge": (
                encrypt_result.get("encrypted_message", "")
                if "encrypt_result" in locals()
                else ""
            ),
        },
    )
    ...
    return render(
        request,
        "adminpanel/pgp_verify.html",
        {"form": form, "challenge": challenge, "user": user},
    )
...
def admin_user_detail(request, username):
    ...
    total_withdrawals_btc = withdrawals.filter(
        status="completed", currency="btc"
    ).aggregate(total=Sum("amount"))["total"] or Decimal("0")

    total_withdrawals_xmr = withdrawals.filter(
        status="completed", currency="xmr"
    ).aggregate(total=Sum("amount"))["total"] or Decimal("0")
    ...
    high_value_txs = transactions.filter(
        amount__gte=Decimal("1.0"), currency="btc"
    ).count()
    if high_value_txs > 5:
    ...
    security_alerts = AuditLog.objects.filter(user=user, flagged=True).order_by(
        "-created_at"
    )[:10]
...
def triple_auth(request):
    ...
    context = {
        "form": form,
        "pgp_challenge_message": request.session.get("pgp_challenge", {}).get(
            "text", ""
        ),
    }
...
def approve_withdrawal(request, withdrawal_id):
    withdrawal = get_object_or_404(
        WithdrawalRequest, id=withdrawal_id, status="pending"
    )
    ...
    if request.method == "POST":
        ...
        if (
            ...
            return render(
                request,
                "adminpanel/approve_withdrawal.html",
                {
        ...
        log_admin_action(
            request.user,
            f"Approved withdrawal #{withdrawal.id} for {withdrawal.user.username}",
        )
    ...
    return render(
        request,
        "adminpanel/approve_withdrawal.html",
        {"withdrawal": withdrawal, "captcha_img": img_b64},
    )
...
def vendors_list(request):
    ...
    vendors = Vendor.objects.all().select_related("user").order_by("-trust_level")
...
def disputes_list(request):
    ...
    disputes = Dispute.objects.all().select_related("order", "raised_by").order_by("-created_at")
...
def withdrawal_management(request):
    ...
    stats = {
        ...
        "manual_review_count": WithdrawalRequest.objects.filter(
...
def withdrawal_approve(request, withdrawal_id):
    """Approve withdrawal with PGP challenge and atomic transactions"""
    import hashlib
    import secrets
    ...
    if request.method == "POST":
        ...
        if pgp_response and challenge_id and expected_challenge:
            if expected_challenge in pgp_response:
                with transaction.atomic():
                    withdrawal = WithdrawalRequest.objects.select_for_update().get(
                        id=withdrawal_id
                    )
                    if withdrawal.status in ["pending", "reviewing"]:
                        withdrawal.status = "approved"
                        withdrawal.approved_by = request.user
                        withdrawal.approved_at = timezone.now()

                        audit_data = f"{withdrawal_id}:{withdrawal.amount}:{request.user.username}:{timezone.now().isoformat()}"
                        audit_hmac = hashlib.sha256(
                            f"AUDIT_KEY:{audit_data}".encode()
                        ).hexdigest()
                        withdrawal.admin_notes = (
                            f"PGP verified. HMAC: {audit_hmac[:16]}"
                        )

                        withdrawal.save()

                        request.session.pop(
                            f"withdrawal_challenge_{withdrawal_id}", None
                        )
                        ...
                        logger = logging.getLogger("marketplace.admin")
                        logger.info(
                            f"Admin {request.user.username} approved withdrawal {withdrawal_id} with PGP verification "
                        ...
                        messages.success(
                            request,
                            f"Withdrawal #{withdrawal_id} approved successfully with PGP verification.",
                        )
                        return redirect(
                            "adminpanel:withdrawal_detail", withdrawal_id=withdrawal_id
                        )
                    else:
                        messages.error(
                            request, "Withdrawal cannot be approved in current status."
                        )
            else:
                messages.error(
                    request, "PGP challenge verification failed - invalid response."
                )
        else:
            messages.error(
                request, "PGP challenge verification failed - missing or invalid data."
            )

    # Generate PGP challenge for admin
    challenge_nonce = secrets.token_hex(16)
    request.session[f"withdrawal_challenge_{withdrawal_id}"] = challenge_nonce

    challenge_message = f"""WITHDRAWAL APPROVAL CHALLENGE

Withdrawal ID: {withdrawal_id}
Amount: {withdrawal.amount} {withdrawal.currency.upper()}
User: {withdrawal.user.username}
Address: {withdrawal.address}

Challenge Code: {challenge_nonce}

To approve this withdrawal, decrypt this message and paste the challenge code above."""
    ...
    return render(request, "adminpanel/withdrawal_approve_pgp.html", context)
...
def admin_withdrawal_detail(request, withdrawal_id):
    ...
    if request.method == "POST":
        ...
        if action == "approve":
            ...
            messages.success(
                request, f"Withdrawal #{withdrawal.id} approved successfully."
            )
...
def admin_security_logs(request):
    ...
    high_risk_logs = AuditLog.objects.filter(risk_score__gte=60).order_by(
        "-created_at"
    )[:50]
...
def admin_wallet_overview(request):
    ...
    total_escrow_btc = (
        Wallet.objects.aggregate(Sum("escrow_btc"))["escrow_btc__sum"] or 0
    )
    total_escrow_xmr = (
        Wallet.objects.aggregate(Sum("escrow_xmr"))["escrow_xmr__sum"] or 0
    )
    ...
    recent_discrepancies = WalletBalanceCheck.objects.filter(
        discrepancy_found=True, resolved=False
