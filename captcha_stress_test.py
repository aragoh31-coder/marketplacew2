...
class CaptchaStressTester:
    ...
    async def fetch_challenge(self, session):
        ...
        try:
            async with session.get(
                f"{self.base_url}/anti_ddos/challenge/", timeout=10
            ) as response:
                if response.status == 200:
                    html = await response.text()
                    challenge_match = re.search(
                        r'name="challenge_data" value="([^"]+)"', html
                    )
                    start_time_match = re.search(
                        r'name="start_time" value="([^"]+)"', html
                    )
    ...
    async def submit_solution(self, session, challenge_data, start_time, answer):
        ...
        try:
            ...
            async with session.post(
                f"{self.base_url}/anti_ddos/verify/", data=data, timeout=10
            ) as response:
    ...
    async def legitimate_captcha_solve(self, session):
        ...
        status, response = await self.submit_solution(
            session, challenge["challenge_data"], challenge["start_time"], answer
        )
    ...
    async def run_comprehensive_tests(self):
        ...
        await self.run_test_batch(self.legitimate_captcha_solve, 50, "Legitimate Users")

        await self.run_test_batch(self.rapid_captcha_solve, 100, "Rapid Bot Solving")

        await self.run_test_batch(self.wrong_answer_test, 30, "Wrong Answers")
    ...
    def generate_report(self):
        ...
        report = {
            ...
            "summary": {
                ...
                "avg_solve_time": (
                    sum(r.avg_solve_time for r in self.results) / len(self.results)
                    if self.results
                    else 0
                ),
            },
...
"""
        ...
        legitimate_success = next(
            (r.success_rate for r in self.results if "Legitimate" in r.test_type), 0
        )
        bot_success = next(
            (r.success_rate for r in self.results if "Rapid Bot" in r.test_type), 0
        )
