...
if not HMAC_KEY or #HMAC_KEY < 32 then
    ngx.log(ngx.ERR, "HMAC_KEY missing/weak; set env HMAC_KEY to a 32+ char random string")
...
local function hmac_sha256(key, data)
    ...
    for i = 1, blocksize do
        ...
        o_key_pad[i] = string.char(bit.bxor(byte, 0x5c))
        i_key_pad[i] = string.char(bit.bxor(byte, 0x36))
    end
    o_key_pad = table.concat(o_key_pad)
    i_key_pad = table.concat(i_key_pad)

    local sha = resty_sha256:new()
    sha:update(i_key_pad .. data)
    local inner = sha:final()

    local sha2 = resty_sha256:new()
    sha2:update(o_key_pad .. inner)
    return sha2:final()
end
...
local function get_client_fingerprint()
    ...
    return ngx.encode_base64(str.to_hex(hmac_sha256(HMAC_KEY, fingerprint_data)))
end
